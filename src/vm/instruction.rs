...
impl<O> InstructionDict<O> {
    ...
    pub fn a_add(&mut self, cname: &str, param_num: usize, func: InstructionFunc<O>) {
        self.add(self.0.len(), cname, param_num, func);
    }

    pub fn add(&mut self, opcode: usize, cname: &str, param_num: usize, func: InstructionFunc<O>) {
        self.0.insert(
            opcode,
            Instruction {
                opcode: opcode,
    ...
    pub fn get(&self, opcode: usize) -> &Instruction<O> {
        self.0
            .get(&opcode)
            .unwrap_or_else(|| panic!("Can't find instruction with opcode {}", opcode))
    }

    pub fn get_by_cname(&self, cname: &str) -> &Instruction<O> {
        self.0
            .values()
            .find(|ref i| i.cname == cname)
            .unwrap_or_else(|| panic!("Can't find instruction with cname {}", cname))
    }
